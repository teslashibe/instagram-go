...
func accountAdminBurner(t *testing.T) (*instagram.Client, string) {
	...
	for _, name := range []string{"IG_SESSIONID", "IG_CSRFTOKEN", "IG_DS_USER_ID"} {
		if os.Getenv(name) == "" {
			t.Fatalf("INSTAGRAM_ACCOUNT_ADMIN_LIVE_TEST=1 requires %s", name)
		}
	}
	c := newClient(t)
...
func TestIntegration_AccountAdmin_ProfessionalDisplayRestoresBurner(t *testing.T) {
	...
	if !original.IsProfessional {
		t.Skip("SKIP: burner professional-display smoke is inapplicable because the configured burner is not a professional account; no setting was changed")
	}
